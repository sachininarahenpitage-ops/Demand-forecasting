...
Streamlit app: login → upload data → train → evaluate → view report
Users managed via users.txt on GitHub
"""
...
.status-box { border-radius: 6px; padding: 0.8rem 1rem; font-family: 'IBM Plex Mono'; font-size: 0.85rem; }
...
# ── Login helpers ─────────────────────────────────────────────────────────────
def load_users():
    """Load users from users.txt — works locally and on Streamlit Cloud."""
    users = {}
    # Look for users.txt next to app.py
    users_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "users.txt")
    if not os.path.exists(users_path):
        # Fallback: current working directory
        users_path = "users.txt"
    if not os.path.exists(users_path):
        return {"admin": "admin123"}   # default if file missing
...
def show_login():
    ...
    <div style='text-align:center; padding-top:3rem;'>
        ...
        <h1 style='color:#38bdf8; font-family:IBM Plex Mono; margin:0.5rem 0;'>Demand Forecasting AI</h1>
        <p style='color:#64748b;'>Phase 4 — LightGBM Challenger</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        ...
        st.markdown("""
        <p style='text-align:center; color:#475569; font-size:0.75rem; margin-top:1rem;'>
...
def train_one(df, target_col, grain, progress_bar, status_text, offset, span):
    ...
    for i, (tr, va) in enumerate(tscv.split(X)):
        ...
        progress_bar.progress(offset + span * (i + 1) / 3)
...
3. Review results — AI only ships where it beats baseline by ≥ 10%
...
""")
        ...
        if not all_ready:
            ...
            if st.button("🚀 Train & Evaluate", type="primary", use_container_width=True):
                try:
                    ...
                    allres = run_training(daily, weekly, base)
                    st.session_state["allres"] = allres
                    st.success("Training finished! Go to the **Results** tab.")
                except Exception:
                    st.error("Something went wrong during training.")
    ...
    with tab2:
        if "allres" not in st.session_state:
            ...
            avg_imp   = allres[allres.ml_wins]["improvement"].mean() * 100
            ...
            for col, val, label, sub in [
                ...
                (c3, f"{avg_imp:.1f}%",    "Avg improvement","where ML wins"),
            ...
            st.dataframe(display[["item","Grain","Horizon (days)","ML MAE",
...
## Verdict
Ship ML only for the rows above. Every other item keeps its Phase-3 baseline.
"""
            st.markdown(report)
